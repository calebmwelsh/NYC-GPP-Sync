...
## Goal
Read `data/results.json` (produced by `search_data.py`) and download metadata + files for every work item in the result set, concurrently using a thread pool.
...
## Prerequisites
1. Run `search_data.py` to populate `data/results.json`.
...
## Inputs
No CLI arguments — reads `data/results.json` automatically.
...
- If `data/results.json` is missing, the script exits with an error message — run `search_data.py` first.
